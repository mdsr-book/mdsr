...
download.file(url, destfile = "data-raw/electionStatistics.csv")

Elections <- readr::read_csv("data-raw/electionStatistics.csv")
...
Cherry <- read.csv(url)
Cherry <- readr::read_csv("data-raw/cherry.csv")
...
download.file(url, destfile = "data-raw/nciNetwork.csv")
Cancer <- readr::read_csv("data-raw/nciNetwork.csv")
save(Cancer, file = "data/Cancer.rda", compress = "xz")


# VotesS1-tally
# download.file("https://raw.githubusercontent.com/dtkaplan/DCF-2015/master/Book/Sections/Data/VotesS1-tally.csv", "data-raw/VotesS1-tally.csv")
Votes <- read.csv("data-raw/VotesS1-tally.csv", skip = 1) %>%
  rename(bill = VOTE) %>%
...
Parties <- read.csv("data-raw/VotesS1-tally.csv",
